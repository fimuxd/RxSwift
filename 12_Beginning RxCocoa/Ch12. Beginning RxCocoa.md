...
* **ViewController.swift**를 확인해보자. 이 프로젝트에서는 하나의 뷰컨트롤러만이 표시된다. 여기서의 목적은 하나의 뷰컨트롤러를 `ApiController`와 연결시켜서 데이터를 제공하게 하는 것이다. 결과적으로는 아래 그림과 같이 단방향 데이터 흐름을 보이게 된다. 
	...
	<img src = "https://github.com/fimuxd/RxSwift/blob/master/12_Beginning%20RxCocoa/1.dataflow.png?raw=true" height = 50>
...
* 데이터를 가져오기 위해 하기의 코드를 `viewDidLoad`에 입력합시다.
	...
	* 여기까지 작성하면 완료된 모습은 다음과 같다
		...
		<img src = "https://github.com/fimuxd/RxSwift/blob/master/12_Beginning%20RxCocoa/2.appScreen.png?raw=true" height = 400 >
...
* 하나씩 문제를 풀어보자. 먼저 첫번째 컴파일러 경고를 확인해보자. 이 경우는 뷰컨트롤러가 dismiss 되었을 때 구독을 취소하도록, 구독이 disposable 객체를 리턴하도록 해야한다. 이를 위해 하기의 코드를 입력하자.
	...
	* 두번째 문제를 확인해보자. RxCocoa는 Cocoa 프레임워크 위에서 관련 기능들을 모두 구현할 수 있게 해준다. 프레임워크는 `rx` 키워드를 추가함으로서 UIKit들을 사용할 수 있게 한다. 즉, 이 문제에서는 `serachCityName.rx.`를 통해 아래 그림과 같이 사용가능한 메소드를 확인할 수 있다. 
		...
		<img src = "https://github.com/fimuxd/RxSwift/blob/master/12_Beginning%20RxCocoa/3.%20rx.png?raw=true" height = 150>
	...
	* 주석을 따라 확인해보자.
		...
		* 2) 이 작업을 적절한 쓰레드로 이동시키고 데이터를 표시하기 위한 코드다
			* 일단 `MainScheduler`로 이동한 이상, 현재 날씨 데이터들은 UI와 함께 업데이트 되어야 한다. 하기 그림은 해당 코드를 시각적으로 표현한 것이다.
			...
			<img src = "https://github.com/fimuxd/RxSwift/blob/master/12_Beginning%20RxCocoa/4.%20dataflow.png?raw=true" height = 50>
...
* 같은 파일 내 `buildRequest`라는 함수도 있다. 이는 네트워크 리쿼스트를 생성하기 위한 것이다. 이 함수는 RxCocoa의 `NSURLSession` wrapper를 이요해서 네트워크 리퀘스트를 보낸다. 하기와 같은 작동을 한다.
	...
	* JSON 객체로 데이터를 매핑하여 반환한다. 이 부분에 해당하는 것이 이 함수의 `return` 부분이다. 하기 코드를 확인할 것.
		...
		* 이를 그림으로 나타내면 다음과 같다. 아래 그림은 `ApiController` 내부에서 어떤일이 일어나는지 시각적으로 나타낸 것이다.
			...
			<img src = "https://github.com/fimuxd/RxSwift/blob/master/12_Beginning%20RxCocoa/5.%20ApiController.png?raw=true" height = 100>
...
* 바인딩을 이해할 수 있는 가장 쉬운 방법은, 두 개의 연결된 속성에 대한 관계를 생각해보는 것이다.
	...
	<img src = "https://github.com/fimuxd/RxSwift/blob/master/12_Beginning%20RxCocoa/6.%20binding.png?raw=true" height = 80>

	* Producer는 값을 만들어낸다.
	* Receiver는 만들어진 값을 수신하고 처리한다. 

	<img src = "https://github.com/fimuxd/RxSwift/blob/master/12_Beginning%20RxCocoa/7.%20biniding.png?raw=true" height = 105>
	...
	* 주석을 따라 살펴보자.
		* 1) 이렇게 `flatMapLatest` 부분을 변경하므로써, 검색 결과는 재사용 될 수 있으며, 일회용 데이터 소스를 여러번 사용되는 Observable로 변형하게 된다. 
			...
			* 이 작은 변화로, 아래와 같은 사용이 가능하다.
				...
				<img src = "https://github.com/fimuxd/RxSwift/blob/master/12_Beginning%20RxCocoa/8.%20binding.png?raw=true" height = 200>
...
* **참고**: [RxSwift의 traits](https://github.com/fimuxd/RxSwift/blob/master/02_Observables/Ch2_Observables.md#h-traits-사용)와 마찬가지로 RxCocoa의 traits도 작업에 도움이 되는 특별기능일 뿐 사용은 선택적이다.
...
<img src = "https://github.com/fimuxd/RxSwift/blob/master/12_Beginning%20RxCocoa/9.%20binding.png?raw=true" height = 50>
